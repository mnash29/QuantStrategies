...
class IterativeBase:
    """Base class for iterative (event-driven) backtesting of trading.
    strategies."""

    def __init__(self, symbol, start, end, amount, use_spread=True):
        """Construct a new IterativeBase.
    ...
    def get_data(self):
        """Import the data from detailed.csv (source can be changed)."""
        raw = pd.read_csv(
            "detailed.csv", parse_dates=["time"], index_col="time"
        ).dropna()
    ...
    def get_values(self, bar):
        """Return the date, the price and the spread for the given bar."""
    ...
    def print_current_balance(self, bar):
        """Print out the current (cash) balance."""
    ...
    def print_current_position_value(self, bar):
        """Print out the current position value."""
        date, price, spread = self.get_values(bar)
        cpv = self.units * price
        print("{} |  Current Position Value = {}".format(date, round(cpv, 2)))

    def print_current_nav(self, bar):
        """Print out the current net asset value (nav)."""
        date, price, spread = self.get_values(bar)
        nav = self.current_balance + self.units * price
        print("{} |  Net Asset Value = {}".format(date, round(nav, 2)))

    def close_pos(self, bar):
        """Close out a long or short position (go neutral)."""
        date, price, spread = self.get_values(bar)
        print(75 * "-")
