...
class MLBacktester:
    """Class for the vectorized backtesting of Machine Learning-based
    ...
    def get_data(self):
        """Import the data from five_minute_pairs.csv
    ...
    def prepare_features(self, start, end):
        """Prepare the feature columns for training set and test set."""
    ...
    def test_strategy(self, train_ratio=0.7, lags=5):
        ...
        train_ratio: float (between 0 and 1.0 excl.)
            Splitting the dataset into training set (train_ratio)
            and test set (1 - train_ratio).
        lags: int
            number of lags serving as model features.
        """
        self.lags = lags

        # determining datetime for start, end and split
        # (for training an testing period)
        ...
        self.data_subset["strategy"] = (
            self.data_subset["pred"] * self.data_subset["returns"]
        )

        # determine the number of trades in each bar
        self.data_subset["trades"] = self.data_subset[
            "pred"
        ].diff().fillna(0).abs()
        ...
        perf = self.results["cstrategy"].iloc[
            -1
        ]  # absolute performance of the strategy
        outperf = (
            perf - self.results["creturns"].iloc[-1]
    ...
    def plot_results(self):
        ...
        if self.results is None:
            ...
            title = "Logistic Regression: {} | TC = {}".format(self.symbol,
                                                               self.tc)
