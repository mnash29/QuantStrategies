...
class ConBacktester:
    """Class for the vectorized backtesting of simple contrarian
    trading strategies.
    """

    def __init__(self, symbol, start, end, tc):
        """Initialize a new ConBacktester.
    ...
    def __repr__(self):
        return "ConBacktester(symbol = {}, start = {}, end = {})".format(
    ...
    def test_strategy(self, window=1):
        ...
        data["position"] = -np.sign(
            data["returns"].rolling(self.window).mean()
        )
    ...
    def plot_results(self):
        """Plot the performance of the trading strategy and compares
        to "buy and hold"."""
        if self.results is None:
            print("Run test_strategy() first.")
        else:
            title = "{} | Window = {} | TC = {}".format(
                self.symbol, self.window, self.tc
            )
    ...
    def optimize_parameter(self, window_range):
        ...
        many_results = pd.DataFrame(
            data={"window": windows, "performance": results}
        )
