...
class MeanRevBacktester:
    """Class for the vectorized backtesting of Bollinger Bands-based
    trading strategies."""

    def __init__(self, symbol, SMA, dev, start, end, tc):
        """Contruct a new MeanRevBacktester.
    ...
    def __repr__(self):
        ...
        return rep.format(self.symbol, self.SMA,
                          self.dev, self.start,
                          self.end)

    def get_data(self):
        """Import the data from intraday_pairs.csv (source can be changed)."""

        raw = pd.read_csv("intraday_pairs.csv",
                          parse_dates=["time"],
                          index_col="time")
    ...
    def set_parameters(self, SMA=None, dev=None):
        """Update parameters (SMA, dev) and the prepared dataset."""
        if SMA is not None:
            self.SMA = SMA
    ...
    def plot_results(self):
        ...
        if self.results is None:
            ...
            title = "{} | SMA = {} | dev = {} | TC = {}".format(
                self.symbol, self.SMA, self.dev, self.tc
            )
    ...
    def optimize_parameters(self, SMA_range, dev_range):
        """Find the optimal strategy (global maximum) given the Bollinger
        Bands parameter ranges.
